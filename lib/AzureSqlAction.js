...
class AzureSqlAction {
    ...
    _executeSqlFile(inputs) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            fileObjs.forEach((file) => __awaiter(this, void 0, void 0, function* () {
                console.log(JSON.stringify(file) + 'XXX');
                yield exec.exec(`"${sqlCmdPath}" -S ${inputs.serverName} -d ${inputs.connectionString.database} -U "${inputs.connectionString.userId}" -P "${inputs.connectionString.password}" -i ${file.toString()} ${inputs.additionalArguments}`);
            }));
